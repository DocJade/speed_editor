...
class SpeedEditor:
	...
	def __init__(self):
		...
		print(self.dev.get_feature_report(6, 10))
	...
	def _parse_report_03(self, report):
		...
		# u8   - Unknown ?
		#print(len(bytearray(report)))
		#print(bytearray(report))
  
		# ending bits always appear to be x00, so we will ignore them.
  
		rid, jm, jv, ju = struct.unpack('<BBiB', bytearray(report)[0:7])
	...
	def _parse_report_04(self, report):
		...
		keys = [SpeedEditorKey(k) for k in struct.unpack('<6H', bytearray(report)[1:]) if k != 0]
	...
	def _parse_report_07(self, report):
		...
		# looks like the report length changed too, cut it off
  
		rid, bs, bl = struct.unpack('<BBB', bytearray(report)[0:3])
